...
if ~strcmp('0',reading{1,1})
    errormsg = 'This file did not come from an ASCII file. Only reads ASCII meshes.';
    fclose('all');
    return;
end
textscan(file_id, '%s',1); % This variable gives the data-size.
reading = textscan(file_id, '%s',1);
...
for readelem = 1:nE
    ...
    readi = textscan(file_id, '%s',1);
    readi = str2double(readi{1,1}{1});
    ...
    elemtype = textscan(file_id, '%s',1);
    elemtype = str2double(elemtype{1,1}{1});
    ...
    tagn = textscan(file_id, '%s',1);
    tagn = str2double(tagn{1,1}{1});
    
    % Reading the tags information
    tagtemp = textscan(file_id, '%s',tagn);
    tagntemp = str2double(tagtemp{1,1}{1});
    ...
    switch (elemtype)
        ...
        case (4) % 4-node tetrahedron
            nE4 = nE4 + 1;
            elemnodes = textscan(file_id, '%s',4);
            
            elements(readelem,1:6) = [4 tagntemp str2double(elemnodes{1,1}{1}) ...
                str2double(elemnodes{1,1}{2}) ...
                str2double(elemnodes{1,1}{3}) ...
                str2double(elemnodes{1,1}{4})];
            
        case (2) % 3-node triangle
            nE2 = nE2 + 1;
            elemnodes = textscan(file_id, '%s',3);
            
            elements(readelem,1:5) = [2 tagntemp str2double(elemnodes{1,1}{1}) ...
                str2double(elemnodes{1,1}{2}) ...
                str2double(elemnodes{1,1}{3})];
            
        case (1) % 2-node line
            nE1 = nE1 + 1;
            elemnodes = textscan(file_id, '%s',2);
            
            elements(readelem,1:4) = [1 tagntemp str2double(elemnodes{1,1}{1}) ...
                str2double(elemnodes{1,1}{2})];
            
        case (15) % 1-node point
            nE15 = nE15 + 1;
            elemnodes = textscan(file_id, '%s',1);
            
            elements(readelem,1:3) = [15 tagntemp str2double(elemnodes{1,1}{1})];
            
        case (3) % 4-node quadrangle
            nE3 = nE3 + 1;
            elemnodes = textscan(file_id, '%s',4);
            
            elements(readelem,1:6) = [3 tagntemp str2double(elemnodes{1,1}{1}) ...
                str2double(elemnodes{1,1}{2}) ...
                str2double(elemnodes{1,1}{3}) ...
                str2double(elemnodes{1,1}{4})];
        ...
        case (5) % 8-node hexahedron
            nE5 = nE5 + 1;
            elemnodes = textscan(file_id, '%s',8);
            
            elements(readelem,:) = [5 tagntemp str2double(elemnodes{1,1}{1}) ...
                str2double(elemnodes{1,1}{2}) ...
                str2double(elemnodes{1,1}{3}) ...
                str2double(elemnodes{1,1}{4}) ...
                str2double(elemnodes{1,1}{5}) ...
                str2double(elemnodes{1,1}{6}) ...
                str2double(elemnodes{1,1}{7}) ...
                str2double(elemnodes{1,1}{8})];
            
            
            
        case (6) % 6-node prism
            nE6 = nE6 + 1;
            elemnodes = textscan(file_id, '%s',6);
            
            elements(readelem,1:8) = [6 tagntemp str2double(elemnodes{1,1}{1}) ...
                str2double(elemnodes{1,1}{2}) ...
                str2double(elemnodes{1,1}{3}) ...
                str2double(elemnodes{1,1}{4}) ...
                str2double(elemnodes{1,1}{5}) ...
        ...
        case (7) % 5-node pyramid
            nE7 = nE7 + 1;
            elemnodes = textscan(file_id, '%s',5);
            
            elements(readelem,1:7) = [7 tagntemp str2double(elemnodes{1,1}{1}) ...
                str2double(elemnodes{1,1}{2}) ...
                str2double(elemnodes{1,1}{3}) ...
                str2double(elemnodes{1,1}{4}) ...
                str2double(elemnodes{1,1}{5})];
